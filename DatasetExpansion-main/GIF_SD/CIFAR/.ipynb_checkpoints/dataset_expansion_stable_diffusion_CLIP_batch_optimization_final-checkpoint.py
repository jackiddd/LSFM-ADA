...
def dataset_expansion(train_loader, model, GIF_model, DDIM_sampler, epoch, use_cuda, class_names, text_classifier,
    ...
    for batch_idx, (original_inputs, augmented_inputs, targets, index) in enumerate(train_loader):
        with torch.no_grad():
            ...
            domain_lsit = ["an image of", "a real-world photo of", "a cartoon image of", "an oil painting of",
                           "a sketch of"]
            adjective_lsit = ["", "colorful", "stylized", "high-contrast", "low-contrast", "posterized", "solarized",
                              "sheared", "bright", "dark"]
            prompts_list = []
            for i in range(expanded_number_per_sample_each_epoch):
                prompts_list.append(
                    "{} {} {}".format(random.choice(domain_lsit), random.choice(adjective_lsit), class_names[targets]))
