...
# 初始化数据集和数据加载器  
train_dataset = ImageFolder(root='/root/NEU-FewShot/data/training-dataset/',
                            transform=transforms.Compose([transforms.ToTensor(),
                                                          transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]))  # 替换[...]为你的数据预处理步骤

train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
  
# 加载预训练的resnet模型，并修改最后一层以匹配你的6个类别  
...
# 保存模型  
torch.save(model.state_dict(), '/root/NEU-FewShot/models/resnet50_training.pth')